    ...
    public class IdentityApiWrapper : IdentityApi
    {
        ...
        public override IMParticleTask<IdentityApiResult> Identify(IdentityApiRequest request = null)
        {
            ...
            MPIdentityApiResultCallback callback = (MPIdentityApiResult apiResult, Foundation.NSError error) =>
            {
                if (error != null)
                {
                    Console.WriteLine($"mParticle Identify Error: {error.LocalizedDescription} (Code: {error.Code})");
                    task.Failure = new IdentityHttpResponseWrapper(error);
                }
                else if (apiResult != null && apiResult.User != null)
                {
                ...
                {
                    // Handle case where apiResult is null or user is null
                    Console.WriteLine("mParticle Identify: No user returned from API");
                    task.Failure = new IdentityHttpResponseWrapper(new Foundation.NSError(new Foundation.NSString("NoUserError"), 0));
                }
            };
