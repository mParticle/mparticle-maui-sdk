...
namespace mParticle.MAUI.iOSBinding {
    ...
    [Protocol, Model]
    [BaseType(typeof(MPExtensionProtocol))]
    interface MPExtensionKitProtocol
    {
    }

    // @interface MPKitRegister : NSObject <MPExtensionKitProtocol>
    [BaseType(typeof(MPExtensionKitProtocol))]
    interface MPKitRegister
    {
        // - (nonnull instancetype)initWithName:(NSString * _Nonnull)name className:(NSString * _Nonnull)className;
        [Export("initWithName:className:")]
        NativeHandle Constructor(string name, string className);
    }
    ...
    interface MParticle
    {
        ...
        // +(void)_setWrapperSdk_internal:(MPWrapperSdk)wrapperSdk version:(NSString * _Nonnull)wrapperSdkVersion;
        [Static]
        [Export("_setWrapperSdk_internal:version:")]
        void SetWrapperSdkInternal(MPWrapperSdk wrapperSdk, string wrapperSdkVersion);

        // @property (readonly, nonatomic, strong) MPRokt * _Nonnull rokt;
        [Export("rokt", ArgumentSemantic.Strong)]
        MPRokt Rokt { get; }
    }
